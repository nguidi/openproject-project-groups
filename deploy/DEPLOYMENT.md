...
```bash
OPENPROJECT_TAG=17-slim ./deploy/build.sh   # → openproject-whispergate:17-slim
```
...
docker build -f deploy/Dockerfile.slim --build-arg OPENPROJECT_TAG=17 \
             -t openproject-whispergate:17-slim --pull .
```
...
> `docker build -f deploy/Dockerfile --build-arg OPENPROJECT_TAG=17 -t openproject-whispergate:17 --pull .`
...
```bash
SMOKE_IMAGE=openproject-whispergate:17-slim \
...
`openproject-whispergate:17`; override with `SMOKE_IMAGE` as above.)
...
Verify it with the same smoke test (`SMOKE_IMAGE=openproject-whispergate:17-slim …`,
...
services:
  web:
    image: openproject-whispergate:17-slim   # was: openproject/openproject:17-slim
    # ...unchanged: env, volumes, depends_on (db, cache, nextcloud), ...
  worker:
    image: openproject-whispergate:17-slim
    # ...
  seeder:
    image: openproject-whispergate:17-slim
    # ...
...
```bash
docker tag openproject-whispergate:17 registry.example.com/openproject-whispergate:17
docker push registry.example.com/openproject-whispergate:17
```
